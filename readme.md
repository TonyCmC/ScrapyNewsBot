...
為了方便接收最新的財經新聞，此專案會定期爬取聯合新聞網(UDN, 經濟日報)、中時電子報、ETtoday新聞雲、自由財經(LTN)、鉅亨網(Cnyes)、Yahoo奇摩股市及工商時報(CTEE，目前停用)等新聞網站的最新一則財經新聞（來源包含 RSS、即時新聞列表頁，或網站前端使用的公開 JSON API），透過 OpenAI（`gpt-4o-mini`）產生摘要並萃取相關股票關鍵字，再以 Telegram Bot API 將整理好的訊息發送至指定的 Telegram 群組/頻道。排程由 APScheduler 負責，各新聞來源以各自的時間間隔輪詢。
