...
A Python bot that crawls the latest finance news from Taiwanese news sites (UDN, ChinaTimes, Ettoday, CTEE, LTN, Cnyes, Yahoo Stock, TrendForce), summarizes and extracts stock-related keywords via OpenAI, and pushes formatted messages to a Telegram group/channel via the Telegram Bot API. Despite the repo name ("ScrapyNewsBot"), the Scrapy framework was removed in a past refactor (see commit `c0a9fca`) — the crawlers are now plain custom classes scheduled with APScheduler. `readme.md` still describes the old Scrapy-based setup and is out of date; trust the code, not the README.
