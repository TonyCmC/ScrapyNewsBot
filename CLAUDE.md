...
A Python bot that crawls the latest finance news from Taiwanese news sites (UDN, ChinaTimes, Ettoday, CTEE, LTN, Cnyes, Yahoo Stock, TrendForce, TechNews), summarizes and extracts stock-related keywords via OpenAI, and pushes formatted messages to a Telegram group/channel via the Telegram Bot API. Despite the repo name ("ScrapyNewsBot"), the Scrapy framework was removed in a past refactor (see commit `c0a9fca`) — the crawlers are now plain custom classes scheduled with APScheduler. `readme.md` still describes the old Scrapy-based setup and is out of date; trust the code, not the README.
...
1. **Crawlers** (`crawlers/`) — each site has its own `*Crawler(NewsCrawler)` subclass implementing `get_latest_news() -> Optional[Dict]`, returning `{'url', 'title', 'content', 'published_at'}` for the single most recent article, or `None` on failure. `crawlers/base_crawler.py` (`NewsCrawler` ABC) provides shared plumbing:
   ...
   - `browser_session()` — a context manager yielding a headless Selenium Chrome `driver`, for JS-rendered sites; paired with `fetch_page_with_browser(driver, url)`. Use this (not `fetch_page`) for sites that render content client-side (currently ChinaTimes, CTEE, LTN use this; UDN, Ettoday, Yahoo, TrendForce, and TechNews use RSS + `fetch_page`; Cnyes calls its frontend's public JSON API directly and needs neither). TrendForce has no single "latest" feed, only per-category RSS feeds, so its crawler polls all of them and keeps whichever entry has the newest `published_parsed`.
...
State is file-based, not a database: dedup history lives in `logs/previous.log` (git-ignored), as `{crawler_name: [url, ...]}` — each source keeps its own rolling last-20-URL window, so a fast-moving source can't evict a slow one's history and cause a re-send. APScheduler's default executor is a `ThreadPoolExecutor`, so different crawlers' jobs genuinely run concurrently; `NewsProcessor._log_lock` (a `threading.Lock`) guards every read-modify-write of `previous.log` to prevent concurrent jobs from clobbering each other's writes. When adding a new crawler, this dedup behavior needs no changes — it's automatically keyed per `crawler_name`.
